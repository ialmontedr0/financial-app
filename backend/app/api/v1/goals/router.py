...
async def create_goal(
    ...
    target_date = (
        date_type.fromisoformat(body["target_date"])
        if isinstance(body.get("target_date"), str) and body.get("target_date")
        else None
    )
    ...
    return await CreateGoalUseCase(db).execute(
        ...
        if body.get("monthly_contribution")
        else None,
        ...
        auto_contribute=body.get("auto_contribute", False),
...
async def list_goals(
    ...
    return await ListGoalsUseCase(db).execute(
        uuid.UUID(current_user["sub"]), goal_type=goal_type, status=status, priority=priority
    )
...
async def update_goal(
    ...
    return await UpdateGoalUseCase(db).execute(
        uuid.UUID(current_user["sub"]), goal_id, changes=body
    )
...
async def get_simulation(
    ...
    return await GetSimulationUseCase(db).execute(
        uuid.UUID(current_user["sub"]), goal_id, simulation_id
    )
...
async def delete_simulation(
    ...
    return await DeleteSimulationUseCase(db).execute(
        uuid.UUID(current_user["sub"]), goal_id, simulation_id
    )
