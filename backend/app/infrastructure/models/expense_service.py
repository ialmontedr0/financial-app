"""Expense Service model - utility providers and billing tracking."""

from __future__ import annotations

import uuid
from datetime import date, datetime  # noqa: TC003
from decimal import Decimal  # noqa: TC003
from typing import TYPE_CHECKING

from sqlalchemy import Boolean, Date, DateTime, ForeignKey, Integer, Numeric, String, Text, func
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import Mapped, mapped_column, relationship

from app.infrastructure.db.base import Base

if TYPE_CHECKING:
    from app.infrastructure.models.category import CategoryModel
    from app.infrastructure.models.user import UserModel


class ExpenseServiceModel(Base):
    """Utility service provider for expense tracking."""

    __tablename__ = "expense_service"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("user.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    category_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("category.id", ondelete="SET NULL"),
        nullable=True,
        default=None,
    )

    # --- Identity ---
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    provider: Mapped[str | None] = mapped_column(
        String(200), nullable=True, default=None
    )  # CEE, INAPA, Altice, etc.
    service_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # electricity, water, gas, internet, phone, cable, other

    # --- Billing ---
    frequency: Mapped[str] = mapped_column(String(20), nullable=False, default="monthly")
    estimated_amount: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=19, scale=4), nullable=True, default=None
    )
    account_number: Mapped[str | None] = mapped_column(
        String(50), nullable=True, default=None
    )  # account number with provider

    # --- Schedule ---
    billing_day: Mapped[int | None] = mapped_column(
        Integer, nullable=True, default=None
    )  # day of month when bill arrives
    due_day: Mapped[int | None] = mapped_column(
        Integer, nullable=True, default=None
    )  # day of month when payment is due

    # --- Payment Status ---
    last_paid_at: Mapped[date | None] = mapped_column(Date, nullable=True, default=None)
    last_paid_amount: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=19, scale=4), nullable=True, default=None
    )
    payment_status: Mapped[str] = mapped_column(String(20), nullable=False, default="pending")

    # --- Settings ---
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    auto_create_expense: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )  # auto-create expense on billing day

    # --- Display ---
    icon: Mapped[str | None] = mapped_column(String(500), nullable=True, default=None)
    color: Mapped[str | None] = mapped_column(String(7), nullable=True, default=None)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True, default=None)

    # --- Timestamps ---
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=func.now(),
        nullable=False,
    )
    deleted_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, default=None
    )

    # --- Relationships ---
    user: Mapped[UserModel] = relationship("UserModel", lazy="noload")
    category: Mapped[CategoryModel | None] = relationship("CategoryModel", lazy="selectin")

    def __repr__(self) -> str:
        return f"<ExpenseServiceModel(id={self.id}, name={self.name}, type={self.service_type})>"
