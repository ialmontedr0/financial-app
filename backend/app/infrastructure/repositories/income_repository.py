...
class IncomeRepository:
    ...
    async def create_income(
        ...
        *,
        ...
        income = IncomeModel(
            ...
            frequency=frequency,
            effective_date=effective_date or date_type.today(),
            notes=notes,
        )
    ...
    async def get_income_by_transaction_id(self, transaction_id: uuid.UUID) -> IncomeModel | None:
        stmt = select(IncomeModel).where(
            IncomeModel.transaction_id == transaction_id,
            IncomeModel.deleted_at.is_(None),
        )
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def update_income(
        self, income_id: uuid.UUID, user_id: uuid.UUID, **kwargs: object
    ) -> IncomeModel | None:
    ...
    async def get_income_summary(self, user_id: uuid.UUID, date_from: date, date_to: date) -> dict:
        ...
        type_stmt = (
            select(
                IncomeModel.income_type,
                func.count(IncomeModel.id).label("count"),
                func.sum(TransactionModel.amount).label("total"),
            )
            .join(TransactionModel, IncomeModel.transaction_id == TransactionModel.id)
            .where(base_filter)
            .group_by(IncomeModel.income_type)
        ...
        stab_stmt = (
            select(
                IncomeModel.stability,
                func.count(IncomeModel.id).label("count"),
                func.sum(TransactionModel.amount).label("total"),
            )
            .join(TransactionModel, IncomeModel.transaction_id == TransactionModel.id)
            .where(base_filter)
            .group_by(IncomeModel.stability)
        )
        by_stability = [
            {"stability": r.stability, "count": r.count, "total": str(r.total or 0)}
            for r in (await self._session.execute(stab_stmt)).all()
        ]

        source_stmt = (
            select(
                IncomeSourceModel.name,
                func.count(IncomeModel.id).label("count"),
                func.sum(TransactionModel.amount).label("total"),
            )
            .join(TransactionModel, IncomeModel.transaction_id == TransactionModel.id)
            .join(
                IncomeSourceModel,
                IncomeModel.income_source_id == IncomeSourceModel.id,
                isouter=True,
            )
            .where(base_filter)
            .group_by(IncomeSourceModel.name)
        )
        by_source = [
            {"source": r.name or "Sin fuente", "count": r.count, "total": str(r.total or 0)}
            for r in (await self._session.execute(source_stmt)).all()
        ]

        months = max(
            1, (date_to.year - date_from.year) * 12 + (date_to.month - date_from.month) + 1
        )
        avg_monthly = total_amount / months

        tax_stmt = select(
            func.sum(IncomeModel.gross_amount).label("gross"),
            func.sum(IncomeModel.tax_withheld).label("withheld"),
        ).where(base_filter)
        ...
        return {
            ...
            "total_income": str(total_amount),
            "total_count": row.count or 0,
            "average_monthly_income": str(round(float(avg_monthly), 2)),
            "gross_income": str(tax_row.gross or 0),
            "total_tax_withheld": str(tax_row.withheld or 0),
    ...
    async def get_income_trends(self, user_id: uuid.UUID, months: int = 12) -> dict:
        ...
        stmt = (
            select(
                func.extract("year", IncomeModel.effective_date).label("year"),
            ...
            .where(
                IncomeModel.user_id == user_id,
                IncomeModel.deleted_at.is_(None),
                IncomeModel.effective_date >= start,
            )
            .group_by(
                func.extract("year", IncomeModel.effective_date),
                func.extract("month", IncomeModel.effective_date),
            )
            .order_by(
                func.extract("year", IncomeModel.effective_date).asc(),
                func.extract("month", IncomeModel.effective_date).asc(),
            )
        )
        monthly_data = [
            {
        ...
        return {
            ...
            "trend": trend,
            "average_monthly": str(round(float(avg), 2)),
    ...
    async def get_income_by_source(
        ...
        stmt = (
            select(
                IncomeSourceModel.id,
                IncomeSourceModel.name,
                IncomeSourceModel.income_type,
                func.count(IncomeModel.id).label("count"),
                func.sum(TransactionModel.amount).label("total"),
            )
            .join(TransactionModel, IncomeModel.transaction_id == TransactionModel.id)
        ...
        return [
            {
                "source_id": str(r.id),
                "source_name": r.name or "Sin fuente",
    ...
    async def get_monthly_breakdown(self, user_id: uuid.UUID, year: int, month: int) -> dict:
        ...
        stmt = (
            select(
                ...
                IncomeModel.income_type,
                IncomeModel.stability,
                IncomeSourceModel.name.label("source_name"),
            )
            .join(TransactionModel, IncomeModel.transaction_id == TransactionModel.id)
            .join(
                IncomeSourceModel,
    ...
    async def detect_recurring_incomes(self, user_id: uuid.UUID) -> list[dict]:
        ...
        stmt = (
            ...
            .order_by(TransactionModel.amount.asc())
        )
    ...
    async def detect_irregular_income(self, user_id: uuid.UUID, months: int = 6) -> list[dict]:
        ...
        sources_stmt = select(IncomeSourceModel).where(
            IncomeSourceModel.user_id == user_id,
            IncomeSourceModel.is_active.is_(True),
            IncomeSourceModel.deleted_at.is_(None),
        )
        sources = list((await self._session.execute(sources_stmt)).scalars().all())
        irregularities = []
        for source in sources:
            stmt = (
                select(TransactionModel.amount, TransactionModel.effective_date)
                .join(IncomeModel, IncomeModel.transaction_id == TransactionModel.id)
                .where(
                    IncomeModel.user_id == user_id,
                    IncomeModel.income_source_id == source.id,
                    IncomeModel.deleted_at.is_(None),
                    TransactionModel.effective_date >= cutoff,
                )
                .order_by(TransactionModel.effective_date.desc())
                ...
                irregularities.append(
                    {
                        ...
                        "type": "missing_income",
                        "message": f"No se detectaron ingresos de '{source.name}'",
                    }
                )
                ...
                irregularities.append(
                    {
                        ...
                        "source_name": source.name,
                        "type": "incomplete",
                        "message": f"Esperados ~{months} pagos, recibidos {len(txs)}",
                    }
                )
    ...
    async def get_income_forecast(self, user_id: uuid.UUID) -> dict:
        ...
        trend = (
            "increasing"
            if avg_3m > avg_6m > avg_12m
            else "decreasing"
            if avg_3m < avg_6m < avg_12m
            else "stable"
        )
        return {
            "average_monthly_3m": str(round(float(avg_3m), 2)),
            "average_monthly_6m": str(round(float(avg_6m), 2)),
            "average_monthly_12m": str(round(float(avg_12m), 2)),
            "trend": trend,
            "projected_next_6m": str(round(float(avg_3m * 6), 2)),
            "projected_monthly": str(round(float(avg_3m), 2)),
        }
    ...
    async def get_source_by_id(
        ...
        stmt = select(IncomeSourceModel).where(
    ...
    async def list_sources(
        self, user_id: uuid.UUID, *, is_active: bool | None = None, income_type: str | None = None
    ) -> list[IncomeSourceModel]:
        stmt = select(IncomeSourceModel).where(
        ...
        return list(
            (await self._session.execute(stmt.order_by(IncomeSourceModel.name.asc())))
            .scalars()
            .all()
        )

    async def update_source(
        self, source_id: uuid.UUID, user_id: uuid.UUID, **kwargs: object
    ) -> IncomeSourceModel | None:
    ...
    ) -> IncomeScheduleModel | None:
        stmt = select(IncomeScheduleModel).where(
            IncomeScheduleModel.id == schedule_id,
            IncomeScheduleModel.user_id == user_id,
            IncomeScheduleModel.deleted_at.is_(None),
        )
        return (await self._session.execute(stmt)).scalar_one_or_none()
    ...
    async def list_schedules(
        ...
        *,
        ...
        return list(
            (await self._session.execute(stmt.order_by(IncomeScheduleModel.expected_date.asc())))
            .scalars()
            .all()
        )

    async def update_schedule(
        self, schedule_id: uuid.UUID, user_id: uuid.UUID, **kwargs: object
    ) -> IncomeScheduleModel | None:
    ...
    async def get_upcoming_schedules(
        ...
        stmt = (
            select(IncomeScheduleModel)
            .where(
                IncomeScheduleModel.user_id == user_id,
                IncomeScheduleModel.status.in_(["projected", "expected"]),
                IncomeScheduleModel.expected_date <= cutoff,
                IncomeScheduleModel.deleted_at.is_(None),
            )
    ...
    async def get_projected_income(self, user_id: uuid.UUID, months: int = 6) -> dict:
        ...
        for s in schedules:
            ...
            total += s.amount
        return {
            "total_projected": str(total),
            "months": months,
            "monthly_breakdown": {k: str(v) for k, v in sorted(monthly.items())},
