...
class GetGoalUseCase:
    ...
    async def execute(self, user_id: uuid.UUID, goal_id: uuid.UUID) -> dict:
        ...
        return {
            ...
            "milestones": [
                {
                    ...
                    "amount_at_event": str(m.amount_at_event),
                    "pct_complete": str(m.pct_complete),
                    "contribution_amount": str(m.contribution_amount)
                    if m.contribution_amount
                    else None,
                    "notes": m.notes,
            ...
            "prediction": {
                "predicted_completion_date": goal.predicted_completion_date.isoformat()
                if goal.predicted_completion_date
                else None,
                "predicted_probability": float(goal.predicted_probability)
                if goal.predicted_probability
                else None,
                "recommended_monthly": str(goal.recommended_monthly)
                if goal.recommended_monthly
                else None,
                "prediction_updated_at": goal.prediction_updated_at.isoformat()
                if goal.prediction_updated_at
                else None,
            },
