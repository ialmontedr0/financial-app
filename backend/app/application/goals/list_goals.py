...
class ListGoalsUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        goals = await self._repo.list_goals(
            user_id, goal_type=goal_type, status=status, priority=priority
        )
        result = []
        for g in goals:
            ...
            result.append(
                {
                    ...
                    "predicted_completion_date": g.predicted_completion_date.isoformat()
                    if g.predicted_completion_date
                    else None,
                    "predicted_probability": float(g.predicted_probability)
                    if g.predicted_probability
                    else None,
                    "recommended_monthly": str(g.recommended_monthly)
                    if g.recommended_monthly
                    else None,
                    "icon": g.icon,
                    "color": g.color,
