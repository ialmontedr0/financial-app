"""Use case: Soft delete a spending limit."""
...
class DeleteSpendingLimitUseCase:
    ...
    async def execute(self, user_id: uuid.UUID, card_id: uuid.UUID, limit_id: uuid.UUID) -> dict:
        ...
        deleted = await self._repo.delete_limit(limit_id, user_id)
        if not deleted:
