"""Use case: Soft-delete a wallet."""

from __future__ import annotations

from typing import TYPE_CHECKING

import structlog

from app.infrastructure.repositories.wallet_repository import WalletRepository

if TYPE_CHECKING:
    import uuid

    from sqlalchemy.ext.asyncio import AsyncSession

logger = structlog.get_logger()


class DeleteWalletUseCase:
    """Soft-delete a wallet. Does NOT delete the linked accounts."""

    def __init__(self, session: AsyncSession) -> None:
        self._session = session
        self._repo = WalletRepository(session)

    async def execute(self, user_id: uuid.UUID, wallet_id: uuid.UUID) -> dict:
        """Soft-delete the wallet."""
        from app.middleware.error_handler import NotFoundError

        wallet = await self._repo.get_by_id(wallet_id, user_id)
        if wallet is None:
            raise NotFoundError("Wallet")

        await self._repo.soft_delete(wallet_id, user_id)

        logger.info(
            "wallet_deleted",
            user_id=str(user_id),
            wallet_id=str(wallet_id),
            name=wallet.name,
        )

        return {
            "message": f"Wallet '{wallet.name}' eliminada exitosamente",
            "wallet_id": str(wallet_id),
        }
