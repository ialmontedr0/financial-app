"""Expense/Income prediction using LightGBM."""
...
class LightGBMPredictor:
    def __init__(self) -> None:
        ...
        self._model_version = "lgbm_expense_v1.0"
    ...
    def _save_model(self, user_id: str) -> None:
        ...
        path = MODEL_DIR / f"lgbm_{self._target_type}_predictor_{user_id}.joblib"
        joblib.dump(
            {
                "model": self._model,
                "scaler": self._scaler,
        ...
        logger.info("lgbm_predictor_saved", path=str(path), target_type=self._target_type)
    ...
    def load_model(self, user_id: str) -> bool:
        ...
        try:
            ...
            self._model_version = data.get("version", "lgbm_expense_v1.0")
            self._target_type = data.get("target_type", "expense")
            self._is_trained = True
            return True
        except Exception as e:
            logger.error("lgbm_predictor_load_failed", error=str(e))
            return False
