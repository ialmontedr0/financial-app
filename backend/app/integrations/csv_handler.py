...
def validate_rows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            errors.append(
                {"row": row_num, "field": "date", "message": f"Invalid date: {row['date']}"}
            )

        try:
            amount_str = (
                str(row["amount"]).replace(",", "").replace("$", "").replace("RD$", "").strip()
            )
            amount = float(amount_str)
            if amount == 0:
                errors.append(
                    {"row": row_num, "field": "amount", "message": "Amount cannot be zero"}
                )
        except (ValueError, TypeError):
            errors.append(
                {"row": row_num, "field": "amount", "message": f"Invalid amount: {row['amount']}"}
            )
                ...
                errors.append(
                    {
                        "row": row_num,
                        "field": "type",
                        "message": f"Invalid type: {row.get('type', '')}",
                    }
                )
