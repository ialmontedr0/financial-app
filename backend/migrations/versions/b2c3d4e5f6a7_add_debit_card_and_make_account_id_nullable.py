...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('account_id', UUID(as_uuid=True), sa.ForeignKey('financial_account.id', ondelete='CASCADE'), nullable=False, index=True),
    ...
    op.alter_column('credit_card', 'account_id',
    ...
    op.add_column('credit_card', sa.Column('is_multicurrency', sa.Boolean(), nullable=False, server_default='false'))
    op.add_column('credit_card', sa.Column('secondary_currency_code', sa.String(3), nullable=True))
    op.add_column('credit_card', sa.Column('secondary_credit_limit', sa.Numeric(precision=19, scale=4), nullable=True))
    op.add_column('credit_card', sa.Column('secondary_available_credit', sa.Numeric(precision=19, scale=4), nullable=True))

    # Drop existing FK constraint on credit_card.account_id and recreate with ON DELETE SET NULL
    op.drop_constraint(op.f('fk_credit_card_account_id_financial_account'), 'credit_card', type_='foreignkey')
    op.create_foreign_key(
        op.f('fk_credit_card_account_id_financial_account'), 'credit_card', 'financial_account',
    ...
    op.drop_constraint(op.f('fk_transaction_account_id_financial_account'), 'transaction', type_='foreignkey')
    op.create_foreign_key(
        op.f('fk_transaction_account_id_financial_account'), 'transaction', 'financial_account',
        ['account_id'], ['id'], ondelete='SET NULL',
    )
